...
class AplikasiInputProduk:
    ...
    def tambah_varian(self):
        ...
        # Cek apakah produk dengan nama dan varian yang sama sudah ada di data_produk_temp
        for item in self.data_produk_temp:
            if item[1] == nama_produk and item[6] == nama_varian:
                messagebox.showerror("Error", "Produk dengan nama dan varian yang sama sudah ada di data sementara!")
                return

        # Cek apakah produk dengan nama dan varian yang sama sudah ada di database
        existing_product = ambil_produk_berdasarkan_nama_dan_varian(nama_produk, nama_varian)
        if existing_product:
            messagebox.showerror("Error", "Produk dengan nama dan varian yang sama sudah ada di database!")
            return
